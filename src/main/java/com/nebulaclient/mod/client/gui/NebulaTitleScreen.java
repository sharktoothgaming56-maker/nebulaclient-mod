...
public class NebulaTitleScreen extends SpaceTheme.SpaceScreen {
    ...
    @Override
    protected boolean showCornerWordmark() {
        return false; // the big logo image above already brands this screen
    }
    ...
    protected void init() {
        ...
        // No .bold here — every button on this screen matches the styling
        // used across the rest of the menus (previously Singleplayer and
        // Multiplayer were bold while the others weren't, which is what made
        // the main menu look different from everything else).
        nebulaButtons.add(new SpaceTheme.NebulaButton(panelX(), y, w, 22, "Singleplayer",
                () -> this.client.setScreen(new SelectWorldScreen(this))));
        y += 26;
        nebulaButtons.add(new SpaceTheme.NebulaButton(panelX(), y, w, 22, "Multiplayer",
                () -> this.client.setScreen(new NebulaServerListScreen(this))));
        y += 26;
    ...
    protected void renderContent(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        // Soft halo, then the supplied NEBULACLIENT logo image, then the
        // tagline just beneath it. Logo width scales with the window but is
        // clamped so it never dominates small screens.
