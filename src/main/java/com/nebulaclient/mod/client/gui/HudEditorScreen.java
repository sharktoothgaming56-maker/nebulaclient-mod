...
public class HudEditorScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        for (HudInfo info : HudInfo.ALL) {
            ...
            // Corner controls: [C] [-] [+], spaced so they don't overlap
            // the HUD label. 14px each with 3px gaps, right-aligned.
            int plusX = pos[0] + w - 16, minusX = pos[0] + w - 33, colorX = pos[0] + w - 50, ctlY = pos[1] + 2;
            ctx.fill(colorX, ctlY, colorX + 14, ctlY + 14, hudColor); // colour swatch itself
            drawBoxOutline(ctx, colorX, ctlY, 14, 14, 0xFFFFFFFF);
            ctx.fill(minusX, ctlY, minusX + 14, ctlY + 14, 0xC0241238);
            ctx.fill(plusX, ctlY, plusX + 14, ctlY + 14, 0xC0241238);
            ctx.drawTextWithShadow(this.textRenderer, "-", minusX + 5, ctlY + 3, 0xFFB98BFF);
            ctx.drawTextWithShadow(this.textRenderer, "+", plusX + 4, ctlY + 3, 0xFFB98BFF);

            boolean inside = mouseX >= pos[0] && mouseX <= pos[0] + w && mouseY >= pos[1] && mouseY <= pos[1] + h;
            if (justPressed && inside && dragging == null) {
                boolean onColor = mouseX >= colorX && mouseX <= colorX + 14 && mouseY >= ctlY && mouseY <= ctlY + 14;
                boolean onMinus = mouseX >= minusX && mouseX <= minusX + 14 && mouseY >= ctlY && mouseY <= ctlY + 14;
                boolean onPlus = mouseX >= plusX && mouseX <= plusX + 14 && mouseY >= ctlY && mouseY <= ctlY + 14;
                if (onColor) {
                    // Open the dedicated picker for this HUD (live preview,
                    // clear selection — replaces the old blind cycle).
                    this.client.setScreen(new HudColorScreen(this, info.id, info.label));
                } else if (onMinus) {
