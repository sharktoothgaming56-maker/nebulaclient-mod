...
public class HudEditorScreen extends Screen {
    ...
    private boolean renderControlBar(DrawContext ctx, NebulaConfig cfg, int mouseX, int mouseY, boolean justPressed) {
        ...
        clicked |= barButton(ctx, x, barY + 5, 20, 20, "+", mouseX, mouseY, justPressed,
                () -> cfg.setHudScale(selected.id, scale + 0.25f));
        x += 26;
        // Colour: normal dark bar button with a small chip showing the
        // HUD's current colour (the old full-swatch button washed out to
        // near-white and looked wrong).
        boolean hovC = mouseX >= x && mouseX <= x + 64 && mouseY >= barY + 5 && mouseY <= barY + 25;
        ctx.fill(x, barY + 5, x + 64, barY + 25, hovC ? 0xF0432a6e : 0xE02a1845);
        drawBoxOutline(ctx, x, barY + 5, 64, 20, hovC ? 0xFFB98BFF : 0x80B98BFF);
        ctx.fill(x + 5, barY + 10, x + 15, barY + 20, hudColor);
        drawBoxOutline(ctx, x + 5, barY + 10, 10, 10, 0x80FFFFFF);
        ctx.drawTextWithShadow(this.textRenderer, "Colour", x + 19, barY + 11, 0xFFEAE6F7);
        if (justPressed && hovC) {
