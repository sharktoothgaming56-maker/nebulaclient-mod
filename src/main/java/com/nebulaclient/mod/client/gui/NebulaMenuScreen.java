...
public class NebulaMenuScreen extends SpaceTheme.SpaceScreen {
    ...
    protected void init() {
        ...
        baseY = addAction("Colours", baseY, rowPitch,
                () -> this.client.setScreen(new ButtonColorScreen(this)));
        // Bottom-left wordmark: on/off, then its size.
        baseY = addToggle(cfg, com.nebulaclient.mod.client.hud.LogoHud.ID, "NebulaClient Logo", baseY, rowPitch);
        baseY = addLogoSize(cfg, baseY, rowPitch);
    ...
    /** A row that cycles the bottom-left logo size through small steps and shows the current value. */
    private int addLogoSize(NebulaConfig cfg, int baseY, int pitch) {
        SpaceTheme.NebulaButton btn = new SpaceTheme.NebulaButton(
                panelX(), 0, panelWidth(), 22,
                "Logo Size: " + logoLabel(cfg.getLogoScale()), null);
        btn.setAction(() -> {
            float next = nextLogoScale(cfg.getLogoScale());
            cfg.setLogoScale(next);
            btn.setLabel("Logo Size: " + logoLabel(cfg.getLogoScale()));
        });
        rows.add(new Row(btn, baseY));
        return baseY + pitch;
    }

    private static String logoLabel(float s) {
        return String.format("%.2fx", s);
    }

    /** Steps 0.25 -> 0.5 -> 0.75 -> 1.0 -> 1.25 -> 1.5 -> wrap. */
    private static float nextLogoScale(float current) {
        float[] steps = {0.25f, 0.5f, 0.75f, 1.0f, 1.25f, 1.5f};
        for (int i = 0; i < steps.length; i++) {
            if (Math.abs(steps[i] - current) < 0.01f) {
