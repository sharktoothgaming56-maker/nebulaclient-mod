...
public class NebulaMenuScreen extends SpaceTheme.SpaceScreen {
    ...
    protected boolean showWordmark() {
        return false; // requested: no corner NEBULACLIENT text on this menu
    }
    ...
    protected void renderContent(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        // Space-signature rule under the heading, matching the title screen.
        SpaceTheme.drawHeaderAccent(ctx, this.textRenderer, this.width / 2, 33,
                Math.max(40, panelWidth() / 3), NebulaConfig.get().getStrokeColor());
    }
