...
public final class SpaceTheme {
    ...
    public static class NebulaButton {
        ...
        public void render(DrawContext ctx, MinecraftClient client, int mouseX, int mouseY) {
            ...
            // Border + glow follow the player's chosen button colour (Nebula
            // Menu > Button Colour). Fully opaque on hover, ~50% otherwise;
            // the glow line is a faint ~19% version of the same hue.
            int accent = com.nebulaclient.mod.client.config.NebulaConfig.get().getButtonColor();
            int rgb = accent & 0xFFFFFF;
            int border = hover ? (0xFF000000 | rgb) : (0x80000000 | rgb);
