    ...
    private void rebuildRows() {
        ...
        for (KeybindPreset preset : presets) {
            if (y + rowH >= rowTop && y <= listBottom) {
                ...
                int delX = saveX + bw + 4;
                final int fy = y;
                // Clicking the row itself applies the preset (see renderContent);
                // these two stay as explicit buttons since they're destructive.
    ...
    protected void renderContent(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        for (KeybindPreset preset : presets) {
            if (y + rowH >= rowTop && y <= listBottom) {
                ...
                // Clicking the row (anywhere left of the Save/Delete buttons)
                // applies the preset — one click instead of hunting for a
                // separate Apply button.
                int rowClickW = 120;
                boolean hovered = mouseX >= chipX && mouseX <= chipX + rowClickW
                        && mouseY >= y && mouseY <= y + rowH - 2
                        && mouseY >= rowTop && mouseY <= listBottom;
                if (hovered && !active) ctx.fill(chipX - 2, y, chipX + rowClickW, y + rowH - 2, 0x40FFFFFF);
