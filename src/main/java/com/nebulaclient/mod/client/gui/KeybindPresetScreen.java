 ...
 * (vanilla + this mod's) to that set — no separate Apply button, exactly per
 * the brief, and no "(active)" text appears on the row afterwards. Each row
 * also has a small ✕ to delete. Everything is stored locally in
 * config/nebulaclient.json.
 */
    ...
    private void rebuildRows() {
        ...
        for (KeybindPreset preset : presets) {
            if (y + 22 >= ROW_TOP && y <= listBottom) {
                ...
                // Row label is the preset's name only — no "(active)" suffix.
                // The applied set is still shown in bold, which is enough of
                // a hint without the label changing under you when you click.
