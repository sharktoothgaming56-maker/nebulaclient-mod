...
public class NebulaGameMenuScreen extends SpaceTheme.SpaceScreen {
    ...
    protected void init() {
        ...
        nebulaButtons.add(new SpaceTheme.NebulaButton(panelX(), y, full, 22, "Disconnect",
                () -> {
                    // 1.21.11's disconnect is a self-contained, asynchronous
                    // flow: these convenience methods tear the connection
                    // down, show their own progress screen ("Saving world"),
                    // and route to the next screen themselves when finished.
                    // Earlier versions called setScreen(...) right after,
                    // which fought that state machine — the progress screen
                    // sat over the live world forever. So: one call, nothing
                    // else. Our Nebula title screen still takes over via the
                    // AFTER_INIT swap once vanilla lands wherever it lands.
