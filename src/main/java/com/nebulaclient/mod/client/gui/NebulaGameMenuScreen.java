    ...
    protected void init() {
        ...
        nebulaButtons.add(new SpaceTheme.NebulaButton(panelX(), y, full, 22, "Disconnect",
                () -> {
                    ...
                    // THE HANG FIX: both calls above only put up a status
                    // screen — they don't decide what comes next. Vanilla's
                    // own pause menu always sets the follow-up screen right
                    // after disconnecting; without this the "Saving world" /
                    // "Leaving server" screen stays up forever and the only
                    // way out is restarting the game.
                    this.client.setScreen(new NebulaTitleScreen());
                }));
    }
