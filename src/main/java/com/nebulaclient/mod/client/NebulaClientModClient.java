...
public class NebulaClientModClient implements ClientModInitializer {
    ...
    private void swapVanillaScreens(MinecraftClient client) {
        // Unconditional: any tick the VANILLA title or pause menu is
        // showing (including when other mods reopen it), replace it with
        // ours. Our screens are not instanceof the vanilla ones, so this
        // can't loop. The old once-per-session flag left holes where the
        // vanilla pause menu could stick around — that's the "ugly" menu
        // from the report.
        if (client.currentScreen instanceof TitleScreen) {
            client.setScreen(new NebulaTitleScreen());
        } else if (client.currentScreen instanceof GameMenuScreen) {
