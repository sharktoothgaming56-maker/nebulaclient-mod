...
public class NebulaClientModClient implements ClientModInitializer {
    ...
    private boolean multiplayerScreenSwapped = false; // same guard, for vanilla's multiplayer screen -> our server list
    ...
    private void swapVanillaScreens(MinecraftClient client) {
        // Vanilla's multiplayer screen gets swapped for our server list
        // (favourites, filter, drag-to-reorder) wherever it's opened from.
        if (client.currentScreen instanceof net.minecraft.client.gui.screen.multiplayer.MultiplayerScreen) {
            if (!multiplayerScreenSwapped) {
                multiplayerScreenSwapped = true;
                client.setScreen(new com.nebulaclient.mod.client.gui.NebulaServerListScreen(new NebulaTitleScreen()));
            }
            return;
        }
        multiplayerScreenSwapped = false;
