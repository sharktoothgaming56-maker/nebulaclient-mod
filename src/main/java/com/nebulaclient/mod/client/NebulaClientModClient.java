...
import com.nebulaclient.mod.client.hud.InfoHud;
import com.nebulaclient.mod.client.hud.LogoHud;
import com.nebulaclient.mod.client.hud.StatusHud;
import com.nebulaclient.mod.client.hud.WaypointCompassHud;
...
public class NebulaClientModClient implements ClientModInitializer {
    ...
    private void registerHuds() {
        ...
        HudElementRegistry.addLast(Identifier.of(NAMESPACE, "info"), InfoHud::render);
        HudElementRegistry.addLast(Identifier.of(NAMESPACE, "effects"), EffectsHud::render);
        HudElementRegistry.addLast(Identifier.of(NAMESPACE, "waypoint_compass"), WaypointCompassHud::render);
    ...
    private void onClientTick(MinecraftClient client) {
        ...
        while (NebulaKeybinds.CREATE_WAYPOINT.wasPressed()) {
            if (client.player != null) {
                int count = WaypointManager.all().size() + 1;
                WaypointManager.create("Waypoint " + count, client);
                sendFeedback(client, "Waypoint created. Rename/delete it from the Waypoint Menu.");
            }
        }
